			...
			if (actor.get() == m_rename)
			{
				...
				if (ImGui::BeginPopupContextItem(actor->name.c_str(), ImGuiPopupFlags_MouseButtonRight))
				{
					if (ImGui::MenuItem("Duplicate"))
					{
						...
						int number = GetTrailingNumber(actor->name);

						// increment the number if a number existed else add "1" to the name
						newActor->name = (number != -1) ? RemoveTrailingNumber(actor->name) + std::to_string(++number) : actor->name + "1";
						m_selected = newActor;
				...
				if (ImGui::BeginPopupContextWindow())
				{
					...
					// the idea was to add components to the empt actor (add component is not implmeneted)
